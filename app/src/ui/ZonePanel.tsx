...
export default function ZonePanel(): React.ReactElement {
  ...
  const isVilla = zone ? VILLA_ZONE.test(zone.name) : false;
      ...
      <Panel title={zone.name}>
        ...
        {!isVilla && (
          <p className="pt-2 text-[11px] leading-snug text-warn">
            The villa generator covers villa, phase and senior-living zones. Apartment towers and the other uses
            arrive with the next milestones.
          </p>
        )}
      </Panel>
...
function FallBreakdown({ option }: { option: LayoutOption }): React.ReactElement {
  ...
  for (const p of option.plots) counts.set(p.terrain.fallClass, (counts.get(p.terrain.fallClass) ?? 0) + 1);
  return (
    <>
      {[...counts.entries()]
        .sort((a, b) => b[1] - a[1])
        .map(([cls, n]) => (
          <Row key={cls} label={cls.replace(/_/g, ' ')} value={`${n} plots`} />
        ))}
    </>
  );
}
