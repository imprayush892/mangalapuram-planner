...
export interface VillaTypeRange {
  /** Key in client_rules.villa_plots.types. */
  name: string;
  minPlinthSft: number;
  maxPlinthSft: number;
  /** Zone names the client named for this type, as written. */
  zones: string[];
}

export function villaTypes(client: YamlDoc): VillaTypeRange[] {
  const types = pick<Record<string, { plinth_sft?: number[]; zones?: string[] }>>(
    client,
    'villa_plots.types',
    {},
  );
  const out: VillaTypeRange[] = [];
  for (const [name, spec] of Object.entries(types)) {
    const range = spec.plinth_sft;
    if (!Array.isArray(range) || range.length === 0) continue;
    out.push({
      name,
      minPlinthSft: Math.min(...range),
      maxPlinthSft: Math.max(...range),
      zones: spec.zones ?? [],
    });
  }
  return out;
...
export interface VillaDelivery {
  /** Footprint one dwelling gets, after row-housing grouping. */
  footprintPerUnitM2: number;
  footprintPerUnitSft: number;
  floors: number;
  /** Plinth one dwelling gets at that floor count. */
  deliveredPlinthSft: number;
  /** What the programme or the client type asks for, where one applies. */
  wantedPlinthSft: number | null;
  typeName: string | null;
  /** Floors the wanted plinth would need on this footprint. */
  floorsNeeded: number | null;
  /** Share of the plot the wanted plinth would need at `floors`. */
  footprintShareNeeded: number | null;
  /** Coverage that share implies, to compare against the KMBR table. */
  coveragePctNeeded: number | null;
  meetsType: boolean;
}

/**
 * What a plot generated by the 20/30/50 method can actually deliver, against the
 * villa size the client confirmed.
 *
 * The client fixes three things independently — the land split (20/30/50), the
 * density (20 units per acre, cashflow C17) and the villa size (1,750–2,000 sft
 * plinth) — and does not check them against each other. This does. Where they
 * cannot all hold, the numbers say by how much rather than the generator
 * quietly drawing a placeholder footprint and reporting success.
 */
export function villaDelivery(
  sizing: PlotSizing,
  floors: number,
  wantedPlinthSft: number | null,
  typeName: string | null,
): VillaDelivery {
  const perUnitM2 = sizing.grouping > 0 ? sizing.footprintM2 / sizing.grouping : 0;
  const perUnitSft = perUnitM2 * 10.7639;
  const delivered = perUnitSft * floors;

  if (!wantedPlinthSft || wantedPlinthSft <= 0 || perUnitSft <= 0) {
    return {
      footprintPerUnitM2: perUnitM2,
      footprintPerUnitSft: perUnitSft,
      floors,
      deliveredPlinthSft: delivered,
      wantedPlinthSft,
      typeName,
      floorsNeeded: null,
      footprintShareNeeded: null,
      coveragePctNeeded: null,
      meetsType: true,
    };
  }

  const floorsNeeded = wantedPlinthSft / perUnitSft;
  const footprintPerUnitNeededM2 = sftToM2(wantedPlinthSft / floors);
  const shareNeeded =
    sizing.plotAreaM2 > 0 ? (footprintPerUnitNeededM2 * sizing.grouping) / sizing.plotAreaM2 : 0;
  ...
  return {
    ...
    typeName,
    floorsNeeded,
    footprintShareNeeded: shareNeeded,
    // Coverage is measured on the plot, which is the saleable land.
    coveragePctNeeded: shareNeeded * 100,
    meetsType: delivered >= wantedPlinthSft * 0.999,
  };
}

/** Preferred plot facings, client `plot_orientation.preferred_facing`. */
export const preferredFacings = (client: YamlDoc): string[] =>
  require$<string[]>(client, 'plot_orientation.preferred_facing', 'client plot orientation');

/** True where the client wants corner plots larger, `plot_orientation.corner_plots`. */
export const cornerPlotsArePremium = (client: YamlDoc): boolean =>
  pick<string>(client, 'plot_orientation.corner_plots', '') === 'premium';

/** Client recreational area rule, checked against the KMBR subdivision rule. */
export function clientRecreation(client: YamlDoc): { share: number; minWidthM: number } {
  return {
    share: require$<number>(
      client,
      'recreational_area.share_of_development_or_cluster',
      'client recreational area',
    ),
    minWidthM: require$<number>(client, 'recreational_area.min_width_m', 'client recreational area'),
  };
}

/** Client plot aspect band, both ends. */
export function aspectBand(client: YamlDoc): { min: number; max: number } {
  return {
    min: require$<number>(client, 'villa_plots.aspect_ratio.min', 'client plot aspect ratio'),
    max: require$<number>(client, 'villa_plots.aspect_ratio.max', 'client plot aspect ratio'),
  };
}
